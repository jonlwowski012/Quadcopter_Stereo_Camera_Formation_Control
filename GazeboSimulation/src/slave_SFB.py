...
# Description: UAV uses state feedback control to follow a master UAV
...
### State feedback controller function
def mav_controller_SFB():
	...
	while not rospy.is_shutdown():
		...
		### X Pose State Feedback Controller
		p_x = .0003
		ts = 0.001
		slave_velocity_x = p_x*((master_location.position.x/ts)-(uav_location.position.x/ts)+(master_velocity.linear.x)+(master_projection.pose.position.x/ts)-((slave_projection.pose.position.x-0.0)/ts))
		...
		### Y Pose State Feedback Controller
		p_y = .0003
		slave_velocity_y = p_y*((master_location.position.y/ts)-(uav_location.position.y/ts)+(master_velocity.linear.y)+(master_projection.pose.position.y/ts)-((slave_projection.pose.position.y-0.0)/ts))
		...
		if flag == True:
			try:
	    			os.remove('/home/ace/catkin_ws/src/GazeboSimulation/results/SFB/error.csv')
				flag = False
			except OSError:
	    			pass
		with open('/home/ace/catkin_ws/src/GazeboSimulation/results/SFB/error.csv', 'a') as fp:
