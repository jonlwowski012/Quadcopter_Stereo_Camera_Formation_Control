...
def mav_controller_PID():
	...
	while not rospy.is_shutdown():
		...
		# X PD Controller
		master_x = master_location.position.x + master_projection.pose.position.x
		slave_x = uav_location.position.x + slave_projection.pose.position.x
		error_x = master_x - slave_x
		...
		# Y PD Controller
		master_y = master_location.position.y + master_projection.pose.position.y
		slave_y = uav_location.position.y + slave_projection.pose.position.y
		error_y = master_y - slave_y
		...
		if flag == True:
			try:
	    			os.remove('/home/ace/catkin_ws/src/GazeboSimulation/results/PID/error.csv')
				flag = False
			except OSError:
	    			pass
		with open('/home/ace/catkin_ws/src/GazeboSimulation/results/PID/error.csv', 'a') as fp:
