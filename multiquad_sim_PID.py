...
# Define the quads
quads={'q1':{'position':[1,0,4],'orientation':[0,0,0],'L':0.3,'r':0.1,'prop_size':[10,4.5],'weight':1.2,'mprj':[1,0,0],'sprj':[-1,0,0]},
	'q2':{'position':[2,5,4],'orientation':[0,0,0],'L':0.15,'r':0.05,'prop_size':[6,4.5],'weight':0.7,'mprj':[1,0,0],'sprj':[-1,0,0]},
	'q3':{'position':[-2,4,4],'orientation':[0,0,0],'L':0.15,'r':0.05,'prop_size':[6,4.5],'weight':0.7,'mprj':[1,0,0],'sprj':[-1,0,0]}}
...
def form_control():
	...
	while(run==True):
		...
		### Slave 1 Controller	
		# X PID Controller
		slave_point = quad.get_position('q2')[0]+quad.get_projection_calculation('q2',False)[0]
		master_point = quad.get_position('q1')[0]+quad.get_projection_calculation('q1',True)[0]
		error_x = master_point-slave_point
		P_x = Kpx*error_x
		...
		# Y PID Controller
		slave_point = quad.get_position('q2')[1]+quad.get_projection_calculation('q2',False)[1]
		master_point = quad.get_position('q1')[1]+quad.get_projection_calculation('q1',True)[1]
		error_y = master_point-slave_point
		P_y = Kpy*error_y
		...
		### Slave 2 Controller	
		slave_vel_x2 = 0.0005*((quad.get_position('q1')[0]/dt)-(quad.get_position('q3')[0]/dt)+quad.get_linear_rate('q1')[0]+(quad.get_projection_calculation('q1',False)[0]/dt)-(quad.get_projection_calculation('q3',True)[0]/dt))
		slave_vel_y2 = 0.0005*((quad.get_position('q1')[1]/dt)-(quad.get_position('q3')[1]/dt)+quad.get_linear_rate('q1')[1]+(quad.get_projection_calculation('q1',False)[1]/dt)-(quad.get_projection_calculation('q3',True)[1]/dt))
		ctrl3.update_target([slave_vel_x2,slave_vel_y2,2])
		print counter
		time.sleep(0.001)
		dt = time.time()-t0
...
def Run():
	...
	while(run==True):
		...
		with open('/home/ace/catkin_ws/src/Quadcopter_simulator/results/PID/results.csv', 'a') as fp:
